...
import static net.fenn7.thatchermod.commonside.block.custom.ThatcherismAltarBlock.IS_CHANNELING;
import static net.fenn7.thatchermod.commonside.block.custom.ThatcherismAltarBlock.IS_PREPARED;
...
public class ThatcherismAltarBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory {
    ...
    public ThatcherismAltarBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.THATCHERISM_ALTAR.get(), pos, state);
            ...
            if (entity.channelingProgress < entity.maxChannelingProgress) {
                ...
                if (entity.channelingProgress == 170) { //should do this on the final major beat of the song
                    if (!world.isClient()) {
                        ...
                        ModEntities.THATCHER.get().spawn((ServerWorld) world, null, null, null, pos,
    ...
    private static boolean hasRecipe(ThatcherismAltarBlockEntity entity) {
        boolean hasRecipe = (entity.getStack(0).getItem() == ModItems.HEART_OF_THATCHER.get() &&
                entity.getStack(1).getItem() == ModItems.SOUL_OF_THATCHER.get()) ||
                (entity.getStack(0).getItem() == ModItems.SOUL_OF_THATCHER.get() &&
                        entity.getStack(1).getItem() == ModItems.HEART_OF_THATCHER.get());
        return hasRecipe;
    }
