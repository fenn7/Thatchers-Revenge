                ...
                if (!((double) f < 0.1D)) {
                    ...
                    if (!world.isClient) {
                        ...
                        if (isRapidFiring(stack)) {
                            ...
                            arrowEntity.setPos(user.getX(), user.getY() + 1.33D, user.getZ());
                            ...
                            if ((this.getMaxUseTime(stack) - remainingUseTicks) >= 27) { // 20 x number of seconds to zone in fully
                                arrowEntity.setCritical(true); arrowEntity.setNoGravity(true);
                        ...
                        if (j > 0) { arrowEntity.setDamage(arrowEntity.getDamage() + (double) j * 0.5D + 0.5D); }

                        int k = EnchantmentHelper.getLevel(Enchantments.PUNCH, stack);
                        if (k > 0) { arrowEntity.setPunch(k); }
